...
def everything_for_your_cat(cats_data):
    ...
    if not cats_data:
        return ''
    for turn in cats_data:
        cat = f'{turn[0]}, {turn[1]}'
        if turn[2] + ' ' + turn[3] in owner_and_pets:
            owner_and_pets[turn[2] + ' ' + turn[3]].append(cat)
        else:
            owner_and_pets[turn[2] + ' ' + turn[3]] = [cat]

    our_str = []
    for key, value in owner_and_pets.items():
        our_str.append(f'{key}: {"; ".join(value)}')
